...
function createPersonDOMManipulation(person) {
  ...
  line2.innerHTML = `${customers.results[i].location.city}, ${customers.results[
    i
  ].location.state} ${customers.results[i].location.postcode}`;
...
function createPersonWithLiteral(person) {
  let personHTML = `<div class="person">
      ...
      <address>
        ...
        <p>${person.location.city}, ${person.location.state}, ${person.location
    .postcode}</p>
        <p>${person.cell}</p>
      </address>
